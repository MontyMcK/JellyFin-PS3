...
// --- CPU-side geometry sanitisation ---------------------------------------
//
// RSX-local vertex memory is write-only from the PPU.  The hardware strobe
// investigation established that reading WaveVert back from RSX memory is
// unsafe, even when the returned values are subsequently sanitised.
//
// Therefore validation happens HERE, while the finished geometry is still
// ordinary PPU/main memory.  Clean vertices are left bit-for-bit unchanged.
// Invalid vertices collapse onto the nearest valid vertex in the same layer,
// producing zero-area triangles instead of a spike or NaN reaching the RSX.
//
// Invalid means:
//   - ok == 0
//   - any non-finite x/y/z
//   - any coordinate outside the conservative +/-4 clip-space guard band
//
// If a layer contains no valid vertex it is undrawable and the caller must
// drop the layer.  The search is deterministic: nearest distance wins, and
// the lower vertex index wins ties.
//
// Pure C, header-only, deterministic, no libm, no PS3 headers.
static inline int jw_sanitize_layer(jw_vert *v, int *repaired)
{
    int valid[JW_VERTS];
    int i, j, nvalid = 0;
    int fixed = 0;
    ...
    for (i = 0; i < JW_VERTS; i++) {
        ...
        valid[i] = (p->ok && finite && inside) ? 1 : 0;
        if (valid[i]) nvalid++;
    }
    ...
    for (i = 0; i < JW_VERTS; i++) {
        int best = -1;
        int best_dist = JW_VERTS + 1;

        if (valid[i])
            continue;

        for (j = 0; j < JW_VERTS; j++) {
            int d;
            if (!valid[j]) continue;
            d = i > j ? i - j : j - i;
            if (d < best_dist) {
